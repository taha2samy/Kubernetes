...
3. **أبدًا (Never)**: تعني أن الحاوية لن يتم إعادة تشغيلها إطلاقًا بعد توقفها. يُستخدم هذا الخيار غالبًا للمهام التي يُفترض أن تُنفذ مرة واحدة فقط، ولا يُراد إعادة تشغيلها بعد انتهائها.
...
>
> CNI (container network interface) تركز على توفير الاتصال الشبكي بين container، بالإضافة إلى تنظيف وإطلاق الموارد التي تم تخصيصها لها عندما يتم تدمير الحاوية. بسبب هذا التركيز البسيط، تظل CNI سهلة الفهم والاستخدام وتحظى بدعم واسع على الرغم من النمو السريع في تقنيات containers.
>
...
>
> - **ADD**: لإضافة حاوية إلى الشبكة أو تعديل configuration.
>
>   ```bash
>   $ cni-plugin ADD <container-id> <network-name> <network-namespace>
>   ```
>
> - **DEL**: لإزالة الحاوية من الشبكة أو إلغاء configuration.
>
>   ```bash
...
> - **CNI_CONTAINERID**: معرف الحاوية.
> - **CNI_NETNS**: نطاق العزل للحاوية. إذا تم استخدام مساحات أسماء الشبكة، فإن هذه القيمة تكون عنوان مساحة الأسماء.
> - **CNI_IFNAME**: اسم الواجهة التي سيتم إنشاؤها داخل الحاوية، مثل eth0.
...
>
>    \- **ADD**: لإضافة حاوية إلى الشبكة.
>
>    \- **DEL**: لإزالة الحاوية من الشبكة.
>
...
>
> 1. **بيئة تشغيل الحاوية (Container Runtime):**
>    تبدأ بيئة تشغيل الحاوية عملية تكوين الشبكة. وتقوم بإرسال التكوين الأولي للشبكة وتكوين الإضافات إلى **إضافة الواجهة**.
...
>    الإضافة الأولى في السلسلة هي **إضافة الواجهة**، وهي المسؤولة عن إنشاء واجهة الشبكة للحاوية. تستقبل هذه الإضافة التكوين الأولي من بيئة التشغيل وتقوم بمعالجته لإنتاج **الناتج السابق (PrevResult)**، والذي يحتوي على إعدادات الشبكة المطبقة حتى الآن.
> 4. **الإضافة المتسلسلة (Chained Plugin):**
>    يتم تمرير الناتج السابق **(PrevResult)** من إضافة الواجهة، مع تكوين إضافي (Plugin Config)، إلى الإضافة التالية في التسلسل، المعروفة بـ **الإضافة المتسلسلة**. تقوم هذه الإضافة بتعديل أو ضبط واجهة الشبكة الموجودة حسب الحاجة، وقد تقوم بتحديث **الناتج السابق (PrevResult)** بإضافات جديدة.
> 5. **الناتج النهائي:**
>    يتم إرجاع **الناتج السابق (PrevResult)** النهائي من الإضافة المتسلسلة إلى بيئة تشغيل الحاوية. يشمل هذا الناتج التكوين الكامل للشبكة ويتم حفظه بواسطة بيئة التشغيل لاستخدامه في المستقبل، مثل عملية الحذف.
>
...
>
> This diagram illustrates the execution flow of CNI (Container Network Interface) plugins within a container runtime environment. It shows how the network configuration is passed through various stages involving the interface plugin and chained plugins:
>
> **Container Runtime Environment:**
> The container runtime starts the network configuration process. It sends the initial network configuration and plugin configurations to the interface plugin.
>
> **Execution Configuration:**
> This section includes the network configuration settings that specify how the network interface should be set up, as well as the configuration for each plugin used in the process.
>
> **Interface Plugin:**
> The first plugin in the sequence is the interface plugin, responsible for creating the network interface for the container. It receives the initial configuration from the container runtime and processes it to produce the previous result (PrevResult), which contains the network settings applied so far.
>
> **Chained Plugin:**
> The previous result (PrevResult) from the interface plugin, along with additional plugin configurations (Plugin Config), is passed to the next plugin in the chain, known as the chained plugin. This plugin modifies or adjusts the existing network interface as needed and may update the previous result (PrevResult) with new additions.
>
> **Final Output:**
> The final previous result (PrevResult) is returned from the chained plugin to the container runtime environment. This result includes the complete network configuration, which is saved by the runtime for future use, such as during container deletion.
>
> Each step depends on the previous plugin's result (PrevResult) to continue processing, allowing each plugin to build upon the configuration applied by the previous one. The execution order and configuration passing ensure smooth setup and teardown of the container network interfaces.
>
